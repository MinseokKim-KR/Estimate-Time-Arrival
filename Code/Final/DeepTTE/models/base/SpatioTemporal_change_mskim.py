...
#	print('START')
#	print('conv_locs size :', conv_locs.size())
"""	
	index = []
	...
	for index_number in range(len(index)-1):
            lngs = torch.unsqueeze(traj['lngs'][index[index_number]:index[index_number+1]], dim = 2)
            lats = torch.unsqueeze(traj['lats'][index[index_number]:index[index_number+1]], dim = 2)
	    states = self.state_em(traj['states'][index[index_number]:index[index_number+1]].long())
        locs = torch.cat((lngs, lats, states), dim = 2)
"""
        attr_t = torch.unsqueeze(attr_t, dim = 1)
#	print('attr_t size :', attr_t.size())
        expand_attr_t = attr_t.expand(conv_locs.size()[:2] + (attr_t.size()[-1], ))
#	print('expand_attr_t size :', expand_attr_t.size())
...
#	print('lens size :', lens)
	...
	print('packed_inputs size :',  len(packed_inputs))
	print('packed_hiddens size :', len(packed_hiddens))
	print('h_n size :', len(h_n))
	print('c_n size :', len(c_n))
	print('hiddens size :', len(hiddens))
	print('lens size :', len(lens))
	print('END')
