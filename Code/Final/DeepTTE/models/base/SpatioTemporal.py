...
class Net(nn.Module):
    ...
    def __init__(self, attr_size, kernel_size=3, num_filter=32, pooling_method='attention', rnn='lstm', elu=1):
	...
	self.elu = elu
        self.geo_conv = GeoConv.Net(kernel_size=kernel_size, num_filter=num_filter, elu=elu)
    ...
    def attent_pooling(self, hiddens, lens, attr_t):
        # print(self.attr2atten(attr_t).size())
        # print(F.tanh(self.attr2atten(attr_t)).size())
    ...
    def forward(self, traj, attr_t, config):
        ...
        # print(conv_locs.size()[:2])
        # print(attr_t.size()[-1])
        # print(conv_locs.size()[:2] + (attr_t.size()[-1],))
